...
--
-- Supports:
--   * Geometry mode: "H_SHAPE" (identical to NCMagnets h_shape.lua-style quarter-face polygon)
--   * Model fraction: "quarter" (x>=0,y>=0) or "half" (x full, y>=0)
--   * Steel material: "1010 Steel" (default) OR custom BH curve (M1200-100A) from .dat
--   * Exports:
--       - gap_By_scan.csv (By along midplane)
--       - Bx_profile.txt  (Bx along a transverse line, like NCMagnets/B_x_t.txt)
--       - multipoles.csv  (Br Fourier multipoles)
--       - monitor_points.csv (field at user-defined points)
--
-- Assumes: dofile("case_params.lua") already executed in the case folder.
--
-- IMPORTANT:
--   This file is written in "conservative FEMM Lua":
--     - NO generic for ("for ... in ... do")
--     - NO length operator "#"
--     - Avoids ipairs/pairs/gmatch-in-for
--
-- If you hit further parser errors, it's usually because your FEMM build is older/limited.

-- =====================
-- Helpers (FEMM-safe Lua)
-- =====================
...
function split_ws(line)
    ...
    while true do
        ...
        tappend(t, string.sub(line, s, e))
        i = e + 1
    end
    return t
end

-- Load BH curve from a 2-column file: B[T]  H[A/m]
-- Creates a nonlinear steel material by adding BH points.
...
-- Export a 1D Bx profile similar to NCMagnets/B_x_t.txt
...
-- Export monitor points: name,x_mm,y_mm,Bx_T,By_T,Bmag_T
...
-- Mirror a segment list across x=0 by re-adding nodes/segments.
...
-- =====================
-- Geometry: identical H-shape (NCMagnets style)
-- =====================

half_h = h_ap_mm/2
new_w2 = w_pole_mm/2 + dent_pole_w * w_pole_mm/2

new_half_h = half_h + dent_pole_h * h_ap_mm/2
if dent_pole_h < 0 then
    new_half_h = half_h
end
...
-- Draw quarter polygon (FEMM-safe: no #)
npts = tlen(pts)
i = 1
while i <= npts do
    mi_addnode(pts[i].x, pts[i].y)
...
-- If half model: mirror yoke polygon across x=0
...
-- Conductor rectangle in quarter-face window
cond = {
...
-- Mirror conductor for half model (two coils)
if MODEL_FRACTION == "half" then
    i = 1
    while i <= ncond do
        mi_addnode(-cond[i].x, cond[i].y)
...
-- Air in aperture
-- mi_addblocklabel(w_pole_mm/4, half_h/2)
-- mi_selectlabel(w_pole_mm/4, half_h/2)
...
-- Steel yoke label (inside yoke)
local_yx = (new_w2 + c_w_mm + dx_mm) + w_leg_mm*0.5
local_yy = half_h + (c_h_mm + dx_mm + w_leg_mm)*0.5
...
-- Auto-close FEMM for batch runs
if (mi_close) then mi_close() end
